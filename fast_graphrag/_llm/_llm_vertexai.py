...
class VertexAILLMService(BaseLLMService):
    ...
    model: Optional[str] = field(default="gemini-1.5-flash-002")

    def __post_init__(self):
        assert(self.model != None)
        logger.debug("Initialized VertexAILLMService with patched VertexAI client.")
        self.llm_async_client: instructor.Instructor = instructor.from_vertexai(
            client=GenerativeModel(self.model),
            _async=True,
        )
    ...
    async def send_message(
        ...
        **kwargs: Any,
        ...
        llm_response: GTResponseModel = await self.llm_async_client.chat.completions.create(
            ...
            response_model=response_model.Model # type: ignore
...
class VertexAIEmbeddingService(BaseEmbeddingService):
    ...
    model: Optional[str] = field(default="text-embedding-005")
